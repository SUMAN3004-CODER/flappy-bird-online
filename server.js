...
if (!GOOGLE_CLIENT_ID || !MONGO_CONNECTION_STRING || !GOOGLE_CLIENT_SECRET || !SESSION_SECRET) {
    console.error("\n*** FATAL ERROR: Server cannot start. One or more required environment variables are missing. ***\n");
...
const onlineUsers = {}; 
const activeGames = {}; // Track active games to send scores
...
app.get('/auth/logout', (req, res) => {
    if (req.user && onlineUsers[req.user.socketId]) delete onlineUsers[req.user.socketId];
...
io.on('connection', async (socket) => {
    ...
    if (user) {
        ...
        console.log(`User connected: ${user.customUsername || user.displayName}`);
    }

    socket.on('guestLogin', () => {
        const guestId = `guest_${new ObjectId()}`;
        const guestUser = { _id: guestId, customUsername: `Guest-${guestId.slice(-4)}`, isGuest: true, socketId: socket.id };
        onlineUsers[socket.id] = guestUser;
    ...
    socket.on('requestInitialData', async () => {
        ...
        const friends = await usersCollection.find({ _id: { $in: friendIds } }).project({ customUsername: 1, _id: 1, socketId: 1 }).toArray();
        const onlineFriendIds = Object.values(onlineUsers).filter(u => !u.isGuest && friendIds.some(fId => fId.equals(u._id))).map(u => u._id.toString());
        socket.emit('friendsList', { friends, onlineFriendIds });
    });
    ...
    socket.on('addFriend', async (friendId) => {
        try {
            ...
            const recipientSocketId = Object.values(onlineUsers).find(u => u._id.equals(recipient._id))?.socketId;
            socket.emit('friendAdded', { _id: recipient._id, customUsername: recipient.customUsername });
            if (recipientSocketId) {
                io.to(recipientSocketId).emit('friendAdded', { _id: requester._id, customUsername: requester.customUsername });
            }
        } catch (err) { console.error("Error adding friend:", err); }
    });
    
    socket.on('sendInvite', ({ toUserId }) => {
    ...
    socket.on('scoreUpdate', (score) => {
        const user = onlineUsers[socket.id];
        if (!user) return;
        const game = Object.values(activeGames).find(g => g.player1.id.equals(user._id) || g.player2.id.equals(user._id));
        if (!game) return;
        const opponent = game.player1.id.equals(user._id) ? game.player2 : game.player1;
        if(opponent && opponent.socketId) io.to(opponent.socketId).emit('opponentScoreUpdate', score);
    });

    socket.on('gameOver', async ({ score, mode, won, gameId }) => {
        const user = onlineUsers[socket.id];
        if (gameId && activeGames[gameId]) delete activeGames[gameId];
        if (!user || user.isGuest) return;
        if (mode === 'single') {
...
async function getSocketUser(socket) {
    try {
        ...
        if (session && session.passport && session.passport.user) {
            return await usersCollection.findOne({ _id: new ObjectId(session.passport.user) });
        }
